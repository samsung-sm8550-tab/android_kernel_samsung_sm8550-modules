...
static struct snd_soc_card snd_soc_card_kalama_msm;
...
static int kalama_tdm_cirrus_init(struct snd_soc_pcm_runtime *rtd)
{
	int i;
...
static const struct of_device_id kalama_asoc_machine_of_match[]  = {
	{ .compatible = "qcom,kalama-asoc-snd",
	  .data = "codec"},
	{ .compatible = "qcom,kalama-asoc-snd-stub",
	  .data = "stub_codec"},
	{},
};
...
static int kalama_ssr_enable(struct device *dev, void *data)
{
	...
	if (!strcmp(card->name, "kalama-stub-snd-card")) {
		/* TODO */
		dev_dbg(dev, "%s: TODO \n", __func__);
	}
...
static void kalama_ssr_disable(struct device *dev, void *data)
{
	struct platform_device *pdev = to_platform_device(dev);
	struct snd_soc_card *card = platform_get_drvdata(pdev);
...
static const struct snd_event_ops kalama_ssr_ops = {
	.enable = kalama_ssr_enable,
	.disable = kalama_ssr_disable,
};
...
static int msm_audio_ssr_register(struct device *dev)
{
	...
	ret = snd_event_master_register(dev, &kalama_ssr_ops,
					ssr_clients, NULL);
	if (!ret)
...
static struct platform_driver kalama_asoc_machine_driver = {
	.driver = {
		.name = DRV_NAME,
		.owner = THIS_MODULE,
		.pm = &snd_soc_pm_ops,
		.of_match_table = kalama_asoc_machine_of_match,
...
{
	platform_driver_unregister(&kalama_asoc_machine_driver);
}
...
MODULE_DEVICE_TABLE(of, kalama_asoc_machine_of_match);
